...
func TestSignerRotation_SetAnnotations(t *testing.T) {
	var (
		...
		nowCA, err = newTestCACertificate(pkix.Name{CommonName: "creator-tests"}, nowFn)
	)
...
func TestCertificateRotation_NeedNewCertificate(t *testing.T) {
	var (
		...
		invalidNotAfter, _  = time.Parse(time.RFC3339, "")
		invalidNotBefore, _ = time.Parse(time.RFC3339, "")
		nowCA, _            = newTestCACertificate(pkix.Name{CommonName: "creator-tests"}, nowFn)

		twentyMinutesBeforeNow   = time.Now().Add(-20 * time.Minute)
		twentyMinutesBeforeNowFn = func() time.Time { return twentyMinutesBeforeNow }
		twentyMinutesBeforeCA, _ = newTestCACertificate(pkix.Name{CommonName: "creator-tests"}, twentyMinutesBeforeNowFn)
	)
...
func newTestCACertificate(subject pkix.Name, currentTime func() time.Time) (*crypto.CA, error) {
	...
	caCert := &x509.Certificate{
		...
		NotAfter:     currentTime().Add(200 * time.Minute),
