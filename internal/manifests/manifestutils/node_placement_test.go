...
func TestConfigureReplicationWithoutZones(t *testing.T) {
	for _, tc := range []struct {
		...
		{"nil zones", nil},
		{"empty zones", []v1alpha1.ZoneSpec{}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			template := podTemplate()
			require.NoError(t, ConfigureReplication(template, tc.zones, IngesterComponentName, "test"))
...
func TestConfigureReplicationSpreadsAcrossZones(t *testing.T) {
	...
	require.NoError(t, ConfigureReplication(template, zones, IngesterComponentName, "test"))
...
func TestConfigureReplicationExposesAvailabilityZone(t *testing.T) {
	...
	// the init container must be hardened like every other container, otherwise it gets rejected
	// by the restricted pod security standard or by a resource quota
	assert.Equal(t, TempoContainerSecurityContext(), initContainer.SecurityContext)
	assert.Equal(t, podTemplate().Spec.Containers[0].Resources, initContainer.Resources)
...
func TestConfigureReplicationSkipsAvailabilityZoneOfGateway(t *testing.T) {
	...
	require.NoError(t, ConfigureReplication(template, zones, GatewayComponentName, "test"))
