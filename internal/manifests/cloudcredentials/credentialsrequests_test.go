...
func TestBuildCredentialsRequest_CreateForTempoStack(t *testing.T) {
	...
	credReqs, err := BuildCredentialsRequest(&stack, stack.Spec.ServiceAccount)
...
func TestBuildCredentialsRequest_NoEnvConfigured(t *testing.T) {
	...
	err := os.Unsetenv("ROLEARN")
	require.NoError(t, err)

	cco, err := BuildCredentialsRequest(&stack, stack.Spec.ServiceAccount)
