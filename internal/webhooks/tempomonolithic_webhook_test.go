...
func TestMonolithicValidate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			warnings: admission.Warnings{"TempoMonolithic instances without multi-tenancy provide no authentication or authorization on the ingest or query paths, and are not supported on OpenShift"},
			errors:   field.ErrorList{},
		},
